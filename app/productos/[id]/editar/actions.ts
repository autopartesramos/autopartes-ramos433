...
async function obtenerConfiguracion(supabase: Awaited<ReturnType<typeof createClient>>) {
...
export async function actualizarProducto(productoId: number, formData: FormData) {
  ...
  const { iva, margenGeneral, margenMl } = await obtenerConfiguracion(supabase);

  const costoFinal = facturado
    ? Math.round(precioCompra * (1 + iva / 100))
    : Math.round(precioCompra);

  const precioGeneral = Math.round(costoFinal * (1 + margenGeneral / 100));
  const precioMl = Math.round(costoFinal * (1 + margenMl / 100));
